...
export default function BookingPublic() {
  ...
  // Buscar restaurante público sem autenticação
  const { data: restaurants, isLoading: loadingRestaurant } = useQuery({
    queryKey: ['booking-public-restaurant', slug],
    queryFn: async () => {
      try {
        // Buscar restaurante público usando filter direto no slug e public=true
        const result = await restaurantService.filter({
          slug: slug,
          public: true
        });
  ...
  const { data: shifts } = useQuery({
    queryKey: ['booking-public-shifts', restaurant?.id],
    queryFn: async () => {
    ...
    try {
      ...
      if (finalData.customer_id) {
        ...
        if (customer.length === 0) {
          // Converter birth_date para ISO-8601 se fornecido
      ...
      // Buscar TODAS as reservas e filtrar apenas as ativas
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            >
              <div className="flex flex-col items-center text-center h-full justify-between py-8 px-6">
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#C47B3C]/20 to-[#A56A38]/10 flex items-center justify-center mb-6">
