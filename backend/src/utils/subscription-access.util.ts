...
export function normalizeSubscriptionProvider(provider: unknown): string {
  return String(provider || '').trim().toUpperCase();
}

export function hasRhizoRegistration(user?: any, subscription?: any): boolean {
  return Boolean(
...
export function isRhizoSubscription(subscription?: any): boolean {
  return Boolean(
    subscription?.rhizo_customer_id ||
      normalizeSubscriptionProvider(subscription?.provider) === 'RHIZO'
  );
...
export function isBlockedRhizoSubscription(subscription?: any): boolean {
  return isRhizoSubscription(subscription) &&
    BLOCKED_ACCESS_STATUSES.has(normalizeSubscriptionStatus(subscription?.status));
}

export function hasSubscriptionAccess(subscription?: any, user?: any, now = new Date()): boolean {
